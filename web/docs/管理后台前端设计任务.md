# 管理后台前端开发任务清单

> **项目技术栈**：Vue 3 + TypeScript + Vite + Element Plus + Tailwind CSS + Pinia + Vue Router
>
> **设计目标**：现代商务科技感、高颜值、流畅交互、响应式布局
>
> **UI/UX 工具**：使用 **ui-ux-pro-max** skill 进行页面美化和交互优化
>
> **代码规范**：
> - ✅ 所有 TypeScript 类型定义必须添加详细的中文注释
> - ✅ 所有函数/方法必须添加 JSDoc 风格的中文注释
> - ✅ 关键业务逻辑必须添加行内中文注释说明
> - ✅ 接口参数和返回值必须详细说明
>
> **任务组织原则**：按照 基础架构 → 认证布局 → 通用业务 → 管理业务 → 细节打磨 的顺序逐步实现

---

## 📋 任务总览

- **阶段一**：项目初始化与基础架构（任务 1-4）**已完成**
- **阶段二**：认证与布局系统（任务 5-7）**已完成**
- **阶段三**：通用业务模块（任务 8-9）**已完成**
- **阶段四**：管理员核心业务（任务 10-13）**已完成**
- **阶段五**：应用管理与扩展（任务 14-15）
- **阶段六**：UI/UX 极致优化（任务 16-18）

---

## 📝 代码注释规范

### 1️⃣ TypeScript 类型定义注释

**必须包含**：
- 类型/接口的用途说明（使用 `@description`）
- 每个字段的中文注释（行内注释）

**示例**：
```ts
/**
 * 用户信息接口
 * @description 用户的基本信息
 */
export interface User {
  id: number                    // 用户ID
  username: string              // 用户名
  status: UserStatus            // 用户状态
  role: UserRole                // 用户角色
}
```

---

### 2️⃣ 函数/方法注释

**必须包含**：
- 函数用途说明（使用 `@description`）
- 参数说明（使用 `@param`）
- 返回值说明（使用 `@returns`）
- 特殊情况说明（可选）

**示例**：
```ts
/**
 * 查询用户列表
 * @description 分页查询所有用户（需要管理员权限）
 * @param params 查询参数
 * @param params.page 页码（从1开始）
 * @param params.size 每页数量
 * @returns Promise 返回用户列表
 */
export const getUserList = (params: { page: number; size: number }) => {
  return request.get('/admin/users', { params })
}
```

---

### 3️⃣ 业务逻辑注释

**关键代码块必须添加注释**：
- 权限判断逻辑
- 数据转换逻辑
- 复杂的业务流程

**示例**：
```ts
// 权限验证：检查用户角色是否有权限访问当前路由
if (to.meta.roles && !to.meta.roles.includes(userStore.role)) {
  next('/forbidden')    // 无权限，跳转到403页面
  return
}
```

---

### 4️⃣ 组件注释

**Vue 组件必须添加**：
- 组件用途说明
- Props 说明
- Emits 说明
- 插槽说明（如有）

**示例**：
```vue
<script setup lang="ts">
/**
 * 统计卡片组件
 * @description 用于展示单个统计数据，支持趋势显示
 */

interface Props {
  title: string         // 标题
  value: number         // 数值
  trend?: 'up' | 'down' // 趋势（可选）
}

const props = defineProps<Props>()
</script>
```

---

### 5️⃣ 常量和配置注释

**配置项必须说明用途**：

```ts
// API基础地址（从环境变量读取）
export const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || '/api/v1'

// 默认每页显示数量
export const DEFAULT_PAGE_SIZE = 20

// Token在localStorage中的key
export const TOKEN_KEY = 'user_token'
```

---

## 阶段一：项目初始化与基础架构

### ✅ 任务 1：项目脚手架与配置

**目标**：构建稳健的开发环境

**操作清单**：

- [x] 使用 Vite 初始化 Vue 3 + TypeScript 项目
  ```bash
  npm create vite@latest admin-frontend -- --template vue-ts
  cd admin-frontend
  npm install
  ```

- [x] 配置 `tsconfig.json`（路径别名 `@/*`）
  ```json
  {
    "compilerOptions": {
      "baseUrl": ".",
      "paths": {
        "@/*": ["src/*"]
      }
    }
  }
  ```

- [x] 安装并配置 Tailwind CSS
  ```bash
  npm install -D tailwindcss postcss autoprefixer
  npx tailwindcss init -p
  ```
  
  配置 `tailwind.config.js`:
  ```js
  export default {
    content: ['./index.html', './src/**/*.{vue,js,ts,jsx,tsx}'],
    theme: {
      extend: {
        colors: {
          primary: '#3B82F6',
          secondary: '#8B5CF6',
          success: '#10B981',
          warning: '#F59E0B',
          danger: '#EF4444'
        }
      }
    }
  }
  ```

- [x] 安装 Element Plus 并配置按需自动导入
  ```bash
  npm install element-plus
  npm install -D unplugin-vue-components unplugin-auto-import
  ```

- [x] 配置 ESLint + Prettier 代码规范

- [x] 配置环境变量
  ```env
  # .env.development
  VITE_API_BASE_URL=http://localhost:8000/api/v1
  VITE_APP_TITLE=卡密系统管理后台
  
  # .env.production
  VITE_API_BASE_URL=https://api.example.com/api/v1
  VITE_APP_TITLE=卡密系统管理后台
  ```

---

### ✅ 任务 2：核心依赖库集成

**目标**：集成状态管理与路由

**操作清单**：

- [x] 安装并配置 Pinia
  ```bash
  npm install pinia pinia-plugin-persistedstate
  ```
  
  创建 `src/stores/index.ts`:
  ```ts
  import { createPinia } from 'pinia'
  import piniaPluginPersistedstate from 'pinia-plugin-persistedstate'
  
  const pinia = createPinia()
  pinia.use(piniaPluginPersistedstate)
  
  export default pinia
  ```

- [x] 安装并配置 Vue Router
  ```bash
  npm install vue-router@4
  ```

- [x] 安装 Axios 并封装 HTTP 请求工具
  ```bash
  npm install axios
  ```
  
  创建 `src/utils/request.ts`:
  ```ts
  import axios from 'axios'
  import { ElMessage } from 'element-plus'
  import { useUserStore } from '@/stores/user'
  import router from '@/router'
  
  /**
   * 创建Axios实例
   * @description 配置基础URL和超时时间
   */
  const request = axios.create({
    baseURL: import.meta.env.VITE_API_BASE_URL,    // API基础地址
    timeout: 10000                                  // 请求超时时间（10秒）
  })
  
  /**
   * 请求拦截器
   * @description 在发送请求前自动添加Token到请求头
   */
  request.interceptors.request.use(
    (config) => {
      const userStore = useUserStore()
      // 如果用户已登录，在请求头中添加Authorization
      if (userStore.token) {
        config.headers.Authorization = `Bearer ${userStore.token}`
      }
      return config
    },
    (error) => Promise.reject(error)
  )
  
  /**
   * 响应拦截器
   * @description 统一处理响应和错误
   */
  request.interceptors.response.use(
    (response) => response.data,                    // 直接返回响应数据
    (error) => {
      // 401: 未授权，Token过期或无效
      if (error.response?.status === 401) {
        const userStore = useUserStore()
        userStore.clearToken()                      // 清除本地Token
        router.push('/login')                       // 跳转到登录页
        ElMessage.error('登录已过期，请重新登录')
      } 
      // 403: 无权限
      else if (error.response?.status === 403) {
        router.push('/forbidden')                   // 跳转到403页面
        ElMessage.error('无权限访问')
      } 
      // 其他错误
      else {
        ElMessage.error(error.response?.data?.detail || '请求失败')
      }
      return Promise.reject(error)
    }
  )
  
  export default request
  ```

- [x] 安装图标库
  ```bash
  npm install @element-plus/icons-vue
  ```

---

### ✅ 任务 3：UI 设计系统搭建

**目标**：确立"现代商务科技感"的视觉基调

**⭐ 使用 ui-ux-pro-max skill 美化**

**操作清单**：

- [x] 定义全局 CSS 变量与基础样式（`src/styles/index.css`）
  ```css
  @tailwind base;
  @tailwind components;
  @tailwind utilities;
  
  :root {
    --primary-color: #3B82F6;
    --secondary-color: #8B5CF6;
    --bg-primary: #F9FAFB;
    --bg-secondary: #FFFFFF;
    --text-primary: #111827;
    --text-secondary: #6B7280;
    --border-color: #E5E7EB;
    --shadow-sm: 0 1px 2px 0 rgb(0 0 0 / 0.05);
    --shadow-md: 0 4px 6px -1px rgb(0 0 0 / 0.1);
    --shadow-lg: 0 10px 15px -3px rgb(0 0 0 / 0.1);
  }
  
  /* Glassmorphism 效果 */
  .glass-card {
    background: rgba(255, 255, 255, 0.7);
    backdrop-filter: blur(10px);
    border: 1px solid rgba(255, 255, 255, 0.3);
    box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.15);
  }
  
  /* 渐变背景 */
  .gradient-bg {
    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  }
  
  /* 平滑过渡 */
  * {
    transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
  }
  ```

- [x] 实现 Glassmorphism（毛玻璃）工具类或组件

- [x] 封装通用 UI 组件：
  
  **BaseCard** 组件 (`src/components/BaseCard.vue`):
  ```vue
  <template>
    <div class="base-card">
      <div v-if="title" class="card-header">
        <h3 class="card-title">{{ title }}</h3>
        <div class="card-extra">
          <slot name="extra"></slot>
        </div>
      </div>
      <div class="card-body">
        <slot></slot>
      </div>
    </div>
  </template>
  
  <style scoped>
  .base-card {
    @apply bg-white rounded-xl shadow-md hover:shadow-lg p-6;
  }
  
  .card-header {
    @apply flex justify-between items-center mb-4 pb-4 border-b border-gray-200;
  }
  
  .card-title {
    @apply text-lg font-semibold text-gray-800;
  }
  </style>
  ```
  
  **BaseButton** 组件 (`src/components/BaseButton.vue`):
  ```vue
  <template>
    <button 
      class="base-button"
      :class="[`btn-${type}`, `btn-${size}`]"
      @click="$emit('click')"
    >
      <slot></slot>
    </button>
  </template>
  
  <style scoped>
  .base-button {
    @apply px-4 py-2 rounded-lg font-medium transition-all duration-300;
    @apply hover:scale-105 active:scale-95;
  }
  
  .btn-primary {
    @apply bg-blue-500 text-white hover:bg-blue-600;
  }
  
  .btn-success {
    @apply bg-green-500 text-white hover:bg-green-600;
  }
  
  .btn-danger {
    @apply bg-red-500 text-white hover:bg-red-600;
  }
  </style>
  ```

---

### ✅ 任务 4：TypeScript 类型定义与 API 封装

**目标**：完成前后端接口对接准备

**操作清单**：

- [x] 定义 TypeScript 接口类型 (`src/types/index.ts`)
  
  ```ts
  // ==================== 用户相关类型 ====================
  
  /**
   * 用户角色类型
   * @description 定义用户在系统中的角色
   */
  export type UserRole = 'admin' | 'user' // admin: 管理员，user: 普通用户
  
  /**
   * 用户状态类型
   * @description 定义用户账号的状态
   */
  export type UserStatus = 'normal' | 'banned' // normal: 正常，banned: 封禁
  
  /**
   * 用户信息接口
   * @description 用户的基本信息
   */
  export interface User {
    id: number                    // 用户ID
    username: string              // 用户名
    status: UserStatus            // 用户状态
    role: UserRole                // 用户角色
    created_at: string            // 创建时间（ISO 8601格式）
    last_login_at?: string        // 最后登录时间（可选）
  }
  
  /**
   * 登录请求接口
   * @description 用户登录时提交的数据
   */
  export interface LoginRequest {
    username: string              // 用户名
    password: string              // 密码
    app_key?: string              // 应用标识（可选，默认 "default_app"）
    device_id?: string            // 设备ID（可选，用于多设备管理）
  }
  
  /**
   * 登录响应接口
   * @description 登录成功后服务器返回的数据
   */
  export interface LoginResponse {
    token: string                 // JWT Token，用于后续API认证
    user_status: UserStatus       // 用户状态
    has_card: boolean             // 是否已绑定卡密
    username: string              // 用户名
    role: UserRole                // 用户角色
  }
  
  // ==================== 应用相关类型 ====================
  
  /**
   * 应用状态类型
   * @description 定义应用的启用状态
   */
  export type AppStatus = 'normal' | 'disabled' // normal: 正常，disabled: 禁用
  
  /**
   * 应用信息接口
   * @description 应用的基本信息
   */
  export interface App {
    id: number                    // 应用ID
    app_key: string               // 应用唯一标识
    app_name: string              // 应用名称
    status: AppStatus             // 应用状态
    created_at: string            // 创建时间
  }
  
  // ==================== 卡密相关类型 ====================
  
  /**
   * 卡密状态类型
   * @description 定义卡密的使用状态
   */
  export type CardStatus = 'unused' | 'used' | 'disabled' 
  // unused: 未使用，used: 已使用，disabled: 已禁用
  
  /**
   * 卡密信息接口
   * @description 卡密的详细信息
   */
  export interface Card {
    id: number                    // 数据库ID
    card_id: number               // 卡密ID
    card_key: string              // 卡密字符串（格式：XXXX-XXXX-XXXX-XXXX）
    app_name?: string             // 所属应用名称（可选）
    status: CardStatus            // 卡密状态
    expire_time: string           // 过期时间（ISO 8601格式）
    max_device_count: number      // 最大可绑定设备数
    permissions: string[]         // 权限列表
    bind_devices?: number         // 已绑定设备数（可选）
    remark?: string               // 备注信息（可选）
    created_at: string            // 创建时间
  }
  
  // ==================== 设备相关类型 ====================
  
  /**
   * 设备信息接口
   * @description 绑定设备的详细信息
   */
  export interface Device {
    id: number                    // 设备记录ID
    card_id: number               // 关联的卡密ID
    card_key: string              // 关联的卡密字符串
    user_id?: number              // 关联的用户ID（可选）
    username?: string             // 关联的用户名（可选）
    device_id: string             // 设备唯一标识
    device_name?: string          // 设备名称（可选）
    bind_time: string             // 绑定时间
    last_active_at: string        // 最后活跃时间
    status: 'active' | 'disabled' // 设备状态：active-激活，disabled-禁用
  }
  
  // ==================== 统计数据类型 ====================
  
  /**
   * 统计数据接口
   * @description 系统各项数据的统计信息
   */
  export interface Statistics {
    total_users: number           // 用户总数
    total_cards: number           // 卡密总数
    unused_cards: number          // 未使用卡密数
    used_cards: number            // 已使用卡密数
    disabled_cards: number        // 已禁用卡密数
    total_devices: number         // 设备总数
    active_devices: number        // 活跃设备数
    total_apps: number            // 应用总数
    normal_apps: number           // 正常应用数
    today_new_users: number       // 今日新增用户数
    today_new_cards: number       // 今日新增卡密数
  }
  
  // ==================== 分页相关类型 ====================
  
  /**
   * 分页参数接口
   * @description 列表查询时的分页参数
   */
  export interface PaginationParams {
    page: number                  // 页码（从1开始）
    size: number                  // 每页数量
  }
  
  /**
   * 分页响应接口
   * @description 分页列表的响应结构
   */
  export interface PaginationResponse<T> {
    total: number                 // 总记录数
    page: number                  // 当前页码
    size: number                  // 每页数量
    data: T[]                     // 数据列表
  }
  
  // ==================== 通用响应类型 ====================
  
  /**
   * 通用成功响应接口
   * @description API操作成功的通用响应结构
   */
  export interface SuccessResponse {
    success: boolean              // 操作是否成功
    message: string               // 提示信息
  }
  
  /**
   * 通用错误响应接口
   * @description API操作失败的错误响应结构
   */
  export interface ErrorResponse {
    detail: string                // 错误详情
    status_code?: number          // HTTP状态码（可选）
  }
  ```

- [x] 封装 API 请求函数
  
  **认证 API** (`src/api/auth.ts`):
  ```ts
  import request from '@/utils/request'
  import type { LoginRequest, LoginResponse, User } from '@/types'
  
  /**
   * 用户登录
   * @param data 登录请求数据
   * @returns Promise<LoginResponse> 返回token和用户信息
   */
  export const login = (data: LoginRequest) => {
    return request.post<LoginResponse>('/auth/login', data)
  }
  
  /**
   * 用户注册
   * @param data 注册请求数据（用户名和密码）
   * @returns Promise<User> 返回新注册的用户信息
   */
  export const register = (data: { username: string; password: string }) => {
    return request.post<User>('/auth/register', data)
  }
  
  /**
   * 获取当前登录用户信息
   * @returns Promise<User> 返回当前用户详细信息
   */
  export const getCurrentUser = () => {
    return request.get<User>('/auth/me')
  }
  
  /**
   * 用户登出
   * @returns Promise<void> 登出成功后无返回值
   */
  export const logout = () => {
    return request.post('/auth/logout')
  }
  ```
  
  **卡密 API** (`src/api/card.ts`):
  ```ts
  import request from '@/utils/request'
  import type { Card, Device } from '@/types'
  
  /**
   * 查询我的卡密列表
   * @description 获取当前用户已绑定的所有卡密
   * @returns Promise<{has_card: boolean, cards: Card[]}> 返回卡密列表
   */
  export const getMyCards = () => {
    return request.get<{ has_card: boolean; cards: Card[] }>('/card/my')
  }
  
  /**
   * 绑定卡密到当前账号和设备
   * @param data 绑定请求数据
   * @param data.card_key 卡密字符串
   * @param data.device_id 设备唯一标识
   * @param data.device_name 设备名称（可选）
   * @returns Promise 绑定结果
   */
  export const bindCard = (data: {
    card_key: string
    device_id: string
    device_name?: string
  }) => {
    return request.post('/card/bind', data)
  }
  
  /**
   * 解绑设备
   * @description 将指定设备从卡密中解绑
   * @param data 解绑请求数据
   * @param data.card_id 卡密ID
   * @param data.device_id 设备ID
   * @returns Promise 解绑结果
   */
  export const unbindDevice = (data: {
    card_id: number
    device_id: string
  }) => {
    return request.post('/card/unbind-device', data)
  }
  
  /**
   * 查询卡密详情
   * @description 获取指定卡密的详细信息和绑定设备列表
   * @param cardId 卡密ID
   * @returns Promise<{card: Card, devices: Device[]}> 返回卡密详情和设备列表
   */
  export const getCardDetail = (cardId: number) => {
    return request.get<{
      card: Card
      devices: Device[]
    }>(`/card/${cardId}`)
  }
  ```
  
  **应用 API** (`src/api/app.ts`):
  ```ts
  import request from '@/utils/request'
  import type { App } from '@/types'
  
  /**
   * 查询应用列表
   * @description 获取系统中所有应用的列表（需要管理员权限）
   * @returns Promise<{total: number, apps: App[]}> 返回应用列表
   */
  export const getAppList = () => {
    return request.get<{ total: number; apps: App[] }>('/app/list')
  }
  
  /**
   * 创建新应用
   * @description 创建一个新的应用（需要管理员权限）
   * @param data 创建请求数据
   * @param data.app_name 应用名称
   * @param data.app_key 应用标识（可选，不填则自动生成）
   * @returns Promise 创建结果
   */
  export const createApp = (data: {
    app_name: string
    app_key?: string
  }) => {
    return request.post('/app/create', data)
  }
  
  /**
   * 更新应用状态
   * @description 启用或禁用应用（需要管理员权限）
   * @param appId 应用ID
   * @param status 应用状态（normal-正常，disabled-禁用）
   * @returns Promise 更新结果
   */
  export const updateAppStatus = (appId: number, status: string) => {
    return request.post(`/app/${appId}/status`, { status })
  }
  
  /**
   * 查询应用详情
   * @description 获取指定应用的详细信息（需要管理员权限）
   * @param appId 应用ID
   * @returns Promise<App> 返回应用详情
   */
  export const getAppDetail = (appId: number) => {
    return request.get<App>(`/app/${appId}`)
  }
  ```
  
  **管理员 API** (`src/api/admin.ts`):
  ```ts
  import request from '@/utils/request'
  import type { User, Card, Device, Statistics } from '@/types'
  
  /**
   * 批量生成卡密
   * @description 管理员批量生成卡密（需要管理员权限）
   * @param data 生成请求数据
   * @param data.app_id 所属应用ID
   * @param data.count 生成数量
   * @param data.expire_time 过期时间（ISO 8601格式）
   * @param data.max_device_count 最大可绑定设备数
   * @param data.permissions 权限列表
   * @param data.remark 备注信息（可选）
   * @returns Promise 返回生成的卡密列表
   */
  export const generateCards = (data: {
    app_id: number
    count: number
    expire_time: string
    max_device_count: number
    permissions: string[]
    remark?: string
  }) => {
    return request.post<{
      success: boolean
      message: string
      count: number
      cards: string[]
    }>('/admin/card/generate', data)
  }
  
  /**
   * 查询用户列表
   * @description 分页查询所有用户（需要管理员权限）
   * @param params 查询参数
   * @param params.page 页码（从1开始）
   * @param params.size 每页数量
   * @param params.status 用户状态筛选（可选：normal-正常，banned-封禁）
   * @param params.keyword 关键词搜索（可选，搜索用户名）
   * @returns Promise 返回用户列表
   */
  export const getUserList = (params: {
    page: number
    size: number
    status?: string
    keyword?: string
  }) => {
    return request.get<{
      total: number
      page: number
      size: number
      users: User[]
    }>('/admin/users', { params })
  }
  
  /**
   * 更新用户状态
   * @description 封禁或解封用户（需要管理员权限）
   * @param userId 用户ID
   * @param status 用户状态（normal-正常，banned-封禁）
   * @returns Promise 更新结果
   */
  export const updateUserStatus = (userId: number, status: string) => {
    return request.post(`/admin/user/${userId}/status`, null, {
      params: { status }
    })
  }
  
  /**
   * 查询卡密列表
   * @description 分页查询所有卡密（需要管理员权限）
   * @param params 查询参数
   * @param params.page 页码（从1开始）
   * @param params.size 每页数量
   * @param params.app_id 应用ID筛选（可选）
   * @param params.status 卡密状态筛选（可选：unused-未使用，used-已使用，disabled-已禁用）
   * @param params.keyword 关键词搜索（可选，搜索卡密或备注）
   * @returns Promise 返回卡密列表
   */
  export const getCardList = (params: {
    page: number
    size: number
    app_id?: number
    status?: string
    keyword?: string
  }) => {
    return request.get<{
      total: number
      page: number
      size: number
      cards: Card[]
    }>('/admin/cards', { params })
  }
  
  /**
   * 更新卡密状态
   * @description 禁用或启用卡密（需要管理员权限）
   * @param cardId 卡密ID
   * @param status 卡密状态（unused-未使用，used-已使用，disabled-已禁用）
   * @returns Promise 更新结果
   */
  export const updateCardStatus = (cardId: number, status: string) => {
    return request.post(`/admin/card/${cardId}/status`, { status })
  }
  
  /**
   * 更新卡密权限
   * @description 实时修改卡密的权限配置（需要管理员权限）
   * @param cardId 卡密ID
   * @param permissions 新的权限列表
   * @returns Promise 更新结果
   */
  export const updateCardPermissions = (cardId: number, permissions: string[]) => {
    return request.post(`/admin/card/${cardId}/permissions`, { permissions })
  }
  
  /**
   * 查询设备列表
   * @description 分页查询所有设备（需要管理员权限）
   * @param params 查询参数
   * @param params.page 页码（从1开始）
   * @param params.size 每页数量
   * @param params.card_id 卡密ID筛选（可选）
   * @param params.user_id 用户ID筛选（可选）
   * @param params.status 设备状态筛选（可选：active-激活，disabled-禁用）
   * @returns Promise 返回设备列表
   */
  export const getDeviceList = (params: {
    page: number
    size: number
    card_id?: number
    user_id?: number
    status?: string
  }) => {
    return request.get<{
      total: number
      page: number
      size: number
      devices: Device[]
    }>('/admin/devices', { params })
  }
  
  /**
   * 更新设备状态
   * @description 禁用或启用设备（需要管理员权限）
   * @param deviceId 设备ID
   * @param status 设备状态（active-激活，disabled-禁用）
   * @returns Promise 更新结果
   */
  export const updateDeviceStatus = (deviceId: number, status: string) => {
    return request.post(`/admin/device/${deviceId}/status`, { status })
  }
  
  /**
   * 获取统计数据
   * @description 获取系统各项数据的统计信息（需要管理员权限）
   * @returns Promise<Statistics> 返回统计数据
   */
  export const getStatistics = () => {
    return request.get<Statistics>('/admin/statistics')
  }
  ```

---

## 阶段二：认证与布局系统

### ✅ 任务 5：登录页面开发

**目标**：实现高颜值、安全的登录流程

**⭐ 使用 ui-ux-pro-max skill 美化登录页**

**接口信息**：

```http
POST /api/v1/auth/login
Content-Type: application/json

请求参数:
{
  "username": string,        // 用户名
  "password": string,        // 密码
  "app_key": string,         // 应用标识（可选，默认 "default_app"）
  "device_id": string        // 设备ID（可选，默认生成）
}

返回值:
{
  "token": string,           // JWT Token
  "user_status": "normal" | "banned",
  "has_card": boolean,       // 是否已绑定卡密
  "username": string,
  "role": "admin" | "user"
}
```

**操作清单**：

- [x] 创建登录页面组件 (`src/views/login/index.vue`)
  - 科技感渐变背景
  - 居中毛玻璃卡片
  - 动态粒子效果（可选）
  - 表单验证（用户名最少3位，密码最少6位）

- [x] 创建 User Store (`src/stores/user.ts`)
  ```ts
  import { defineStore } from 'pinia'
  import { login, getCurrentUser, logout } from '@/api/auth'
  import type { LoginRequest, UserRole, UserStatus } from '@/types'
  
  /**
   * 用户状态接口
   * @description 定义用户在Pinia Store中的状态结构
   */
  interface UserState {
    token: string                 // JWT Token
    username: string              // 用户名
    role: UserRole                // 用户角色（admin/user）
    user_status: UserStatus       // 用户状态（normal/banned）
    has_card: boolean             // 是否已绑定卡密
  }
  
  /**
   * 用户状态管理
   * @description 管理用户登录状态、角色权限等信息
   */
  export const useUserStore = defineStore('user', {
    // 状态定义
    state: (): UserState => ({
      token: '',
      username: '',
      role: 'user',
      user_status: 'normal',
      has_card: false
    }),
    
    // 计算属性
    getters: {
      /**
       * 是否已登录
       * @returns {boolean} 有token则表示已登录
       */
      isLoggedIn: (state) => !!state.token,
      
      /**
       * 是否为管理员
       * @returns {boolean} 角色为admin则为管理员
       */
      isAdmin: (state) => state.role === 'admin'
    },
    
    // 操作方法
    actions: {
      /**
       * 用户登录
       * @description 调用登录API并保存用户信息
       * @param loginData 登录请求数据
       */
      async login(loginData: LoginRequest) {
        const data = await login(loginData)
        this.token = data.token
        this.username = data.username
        this.role = data.role
        this.user_status = data.user_status
        this.has_card = data.has_card
      },
      
      /**
       * 清除Token
       * @description 清空所有用户状态，重置为初始值
       */
      clearToken() {
        this.$reset()
      },
      
      /**
       * 用户登出
       * @description 调用登出API并清除本地状态
       */
      async logout() {
        await logout()
        this.clearToken()
      }
    },
    
    // 状态持久化配置
    persist: {
      enabled: true,
      strategies: [
        {
          key: 'user',              // localStorage中的key
          storage: localStorage     // 使用localStorage持久化
        }
      ]
    }
  })
  ```

- [x] 实现登录逻辑
  - 调用登录 API
  - 存储 Token 到 Pinia 和 localStorage
  - 根据角色跳转到对应页面（管理员 → `/dashboard`，普通用户 → `/stats`）

- [x] 实现 loading 状态与错误提示

---

### ✅ 任务 6：整体布局（Layout）开发

**目标**：实现响应式、可扩展的后台框架

**⭐ 使用 ui-ux-pro-max skill 美化布局**

**布局要求**（参考 `管理后台整体布局与交互设计说明.md`）：

- 顶部固定导航栏（高度 64px）
- 左侧可折叠菜单（宽度 256px / 64px）
- 主内容区（动态路由渲染）
- 面包屑导航
- 页面切换过渡动画

**操作清单**：

- [x] 创建布局组件 (`src/layouts/AdminLayout.vue`)
  ```vue
  <template>
    <div class="admin-layout">
      <TopBar />
      <div class="layout-content">
        <SideMenu />
        <AppMain />
      </div>
    </div>
  </template>
  ```

- [x] 开发侧边栏 `SideMenu` (`src/layouts/components/SideMenu.vue`)
  - 支持折叠/展开
  - 多级菜单（手风琴效果）
  - 根据角色动态显示菜单
  - 菜单图标 + 文字
  - 当前路由高亮

- [x] 开发顶部栏 `TopBar` (`src/layouts/components/TopBar.vue`)
  - 左侧：系统名称 + 菜单折叠按钮
  - 右侧：用户信息、角色标签、退出登录

- [x] 开发主内容区 `AppMain` (`src/layouts/components/AppMain.vue`)
  - `<router-view>` 渲染
  - 页面切换动画（Fade / Slide）
  - 面包屑导航

- [x] 实现路由权限守卫 (`src/router/permission.ts`)
  ```ts
  import router from './index'
  import { useUserStore } from '@/stores/user'
  
  /**
   * 全局路由守卫
   * @description 在每次路由跳转前进行权限验证
   */
  router.beforeEach((to, from, next) => {
    const userStore = useUserStore()
    
    /**
     * 白名单处理：登录页
     * 如果已登录，跳转到仪表盘
     * 如果未登录，允许访问登录页
     */
    if (to.path === '/login') {
      if (userStore.isLoggedIn) {
        next('/dashboard')  // 已登录用户不能访问登录页，直接跳转到仪表盘
      } else {
        next()              // 未登录用户可以访问登录页
      }
      return
    }
    
    /**
     * 登录验证
     * 需要登录的页面，未登录用户跳转到登录页
     */
    if (to.meta.requiresAuth && !userStore.isLoggedIn) {
      next('/login')        // 未登录，跳转到登录页
      return
    }
    
    /**
     * 角色权限验证
     * 检查用户角色是否有权限访问当前路由
     */
    if (to.meta.roles && !to.meta.roles.includes(userStore.role)) {
      next('/forbidden')    // 无权限，跳转到403页面
      return
    }
    
    // 验证通过，允许访问
    next()
  })
  ```

---

### ✅ 任务 7：动态菜单与路由配置

**目标**：根据角色动态生成菜单

**操作清单**：

- [x] 定义路由表 (`src/router/index.ts`)
  ```ts
  import { createRouter, createWebHistory } from 'vue-router'
  import AdminLayout from '@/layouts/AdminLayout.vue'
  
  /**
   * 路由配置
   * @description 定义系统所有路由，包括页面路径、权限要求等
   */
  const routes = [
    // 登录页（无需认证）
    {
      path: '/login',
      name: 'Login',
      component: () => import('@/views/login/index.vue')
    },
    // 主应用布局（需要认证）
    {
      path: '/',
      redirect: '/dashboard',                    // 默认重定向到仪表盘
      component: AdminLayout,
      meta: { requiresAuth: true },              // 所有子路由都需要登录
      children: [
        // 仪表盘（仅管理员）
        {
          path: 'dashboard',
          name: 'Dashboard',
          component: () => import('@/views/dashboard/index.vue'),
          meta: { 
            title: '仪表盘',                     // 菜单标题
            icon: 'DataLine',                    // 菜单图标
            roles: ['admin']                     // 允许访问的角色
          }
        },
        // 用户管理（仅管理员）
        {
          path: 'users',
          name: 'Users',
          component: () => import('@/views/user/index.vue'),
          meta: { 
            title: '用户管理', 
            icon: 'User', 
            roles: ['admin'] 
          }
        },
        // 卡密管理（仅管理员）
        {
          path: 'cards',
          name: 'Cards',
          component: () => import('@/views/card/index.vue'),
          meta: { 
            title: '卡密管理', 
            icon: 'Ticket', 
            roles: ['admin'] 
          }
        },
        // 设备管理（仅管理员）
        {
          path: 'devices',
          name: 'Devices',
          component: () => import('@/views/device/index.vue'),
          meta: { 
            title: '设备管理', 
            icon: 'Monitor', 
            roles: ['admin'] 
          }
        },
        // 应用管理（仅管理员）
        {
          path: 'apps',
          name: 'Apps',
          component: () => import('@/views/app/index.vue'),
          meta: { 
            title: '应用管理', 
            icon: 'Grid', 
            roles: ['admin'] 
          }
        },
        // 统计数据（管理员和普通用户都可访问）
        {
          path: 'stats',
          name: 'Stats',
          component: () => import('@/views/stats/index.vue'),
          meta: { 
            title: '统计数据', 
            icon: 'DataAnalysis', 
            roles: ['admin', 'user']            // 两种角色都可以访问
          }
        },
        // 个人中心（管理员和普通用户都可访问）
        {
          path: 'profile',
          name: 'Profile',
          component: () => import('@/views/profile/index.vue'),
          meta: { 
            title: '个人中心', 
            icon: 'User', 
            roles: ['admin', 'user']
          }
        }
      ]
    },
    // 403无权限页面
    {
      path: '/forbidden',
      name: 'Forbidden',
      component: () => import('@/views/error/403.vue')
    },
    // 404页面（匹配所有未定义的路由）
    {
      path: '/:pathMatch(.*)*',
      name: 'NotFound',
      component: () => import('@/views/error/404.vue')
    }
  ]
  
  /**
   * 创建路由实例
   * @description 使用HTML5 History模式
   */
  const router = createRouter({
    history: createWebHistory(),                 // 使用history模式（需要服务器配置）
    routes
  })
  
  export default router
  ```

- [x] 创建菜单生成函数
  ```ts
  // src/utils/menu.ts
  import type { RouteRecordRaw } from 'vue-router'
  import { useUserStore } from '@/stores/user'
  
  /**
   * 生成菜单数据
   * @description 根据路由配置和用户角色动态生成菜单结构
   * @param routes 路由配置数组
   * @returns 符合当前用户权限的菜单列表
   */
  export function generateMenus(routes: RouteRecordRaw[]) {
    const userStore = useUserStore()
    const menus: any[] = []
    
    routes.forEach((route) => {
      // 只处理有title的路由（需要在菜单中显示的路由）
      if (route.meta && route.meta.title) {
        const roles = route.meta.roles as string[]
        
        // 如果没有roles限制，或者用户角色在允许的roles中，则显示该菜单
        if (!roles || roles.includes(userStore.role)) {
          menus.push({
            path: route.path,                    // 路由路径
            name: route.name,                    // 路由名称
            title: route.meta.title,             // 菜单标题
            icon: route.meta.icon,               // 菜单图标
            children: route.children             // 递归处理子菜单
              ? generateMenus(route.children)
              : []
          })
        }
      }
    })
    
    return menus
  }
  ```

- [x] 实现 `v-permission` 指令（可选）
  ```ts
  // src/directives/permission.ts
  import type { Directive } from 'vue'
  import { useUserStore } from '@/stores/user'
  
  /**
   * 权限指令
   * @description 用于按钮级别的权限控制
   * @example <el-button v-permission="['admin']">删除</el-button>
   */
  export const permission: Directive = {
    mounted(el, binding) {
      const userStore = useUserStore()
      const { value } = binding                 // 获取指令绑定的值（允许的角色数组）
      
      // 如果当前用户角色不在允许的角色列表中，则移除该元素
      if (value && !value.includes(userStore.role)) {
        el.parentNode?.removeChild(el)
      }
    }
  }
  ```

---

## 阶段三：通用业务模块（所有用户）**已完成**

### ✅ 任务 8：统计仪表盘（Dashboard）**已完成**

**目标**：直观展示核心数据

**⭐ 使用 ui-ux-pro-max skill 美化仪表盘**

**接口信息**：

```http
GET /api/v1/admin/statistics
Authorization: Bearer {token}

返回值:
{
  "users": {
    "total": number,       // 用户总数
    "normal": number,      // 正常用户数
    "banned": number       // 封禁用户数
  },
  "cards": {
    "total": number,       // 卡密总数
    "unused": number,     // 未使用卡密数
    "used": number,       // 已使用卡密数
    "disabled": number    // 禁用卡密数
  },
  "devices": {
    "total": number,       // 设备总数
    "active": number,     // 活跃设备数
    "disabled": number    // 禁用设备数
  },
  "apps": {
    "total": number,       // 应用总数
    "active": number       // 正常应用数
  }
}
```

**操作清单**：

- [x] 创建仪表盘页面 (`src/views/dashboard/index.vue`)

- [x] 开发统计卡片组件 (`src/views/dashboard/components/StatCard.vue`)
  - 数值展示
  - 趋势图标（↑ ↓）
  - 颜色区分（成功、警告、危险）
  - 图标装饰

- [x] 调用统计接口并展示数据
  - 使用 Grid 布局（4列响应式）
  - 卡片内容：
    - 用户统计（总数、正常、封禁）
    - 卡密统计（总数、未使用、已使用、禁用）
    - 设备统计（总数、活跃、禁用）
    - 应用统计（总数、正常）

- [x] 集成 ECharts 图表
  - **卡密使用情况饼图**：展示未使用、已使用、已禁用比例
  - **用户状态分布柱状图**：对比用户总数、正常用户、封禁用户
  - **设备状态分布环形图**：展示活跃设备和禁用设备比例
  - **数据汇总表格**：以表格形式展示所有关键指标

- [x] 安装 ECharts 库
  ```bash
  npm install echarts@5
  ```

---

### ✅ 任务 9：个人中心（Profile）**已完成**

**目标**：用户自我管理与卡密查看

**⭐ 使用 ui-ux-pro-max skill 美化个人中心**

**接口信息**：

1. **获取用户信息**：
   ```http
   GET /api/v1/auth/me
   Authorization: Bearer {token}
   
   返回值:
   {
     "user_id": number,
     "username": string,
     "status": "normal" | "banned",
     "role": "admin" | "user",
     "app_id": number,
     "device_id": string
   }
   ```

2. **查询我的卡密**：
   ```http
   GET /api/v1/card/my
   Authorization: Bearer {token}
   
   返回值:
   {
     "has_card": boolean,
     "cards": [
       {
         "card_id": number,
         "card_key": string,
         "expire_time": string,
         "permissions": string[],
         "bind_devices": number,
         "max_device_count": number,
         "status": "unused" | "used" | "disabled",
         "remark": string
       }
     ]
   }
   ```

3. **绑定卡密**：
   ```http
   POST /api/v1/card/bind
   Authorization: Bearer {token}
   Content-Type: application/json
   
   请求参数:
   {
     "card_key": string,       // 卡密
     "device_id": string,      // 设备ID
     "device_name": string     // 设备名称（可选）
   }
   
   返回值:
   {
     "success": boolean,
     "message": string,
     "card_info": { ... }
   }
   ```

4. **解绑设备**：
   ```http
   POST /api/v1/card/unbind-device
   Authorization: Bearer {token}
   Content-Type: application/json
   
   请求参数:
   {
     "card_id": number,        // 卡密ID
     "device_id": string       // 设备ID
   }
   
   返回值:
   {
     "success": boolean,
     "message": string
   }
   ```

**操作清单**：

- [x] 创建个人中心页面 (`src/views/profile/index.vue`)

- [x] 开发用户信息卡片
  - 头像（可使用默认头像或 Gravatar）
  - 用户名、角色、状态
  - 注册时间、最后登录时间

- [x] 开发"我的卡密"模块
  - 卡密列表展示（卡片式布局）
  - 卡密状态标签（未使用-灰色、已使用-绿色、禁用-红色）
  - 权限列表展示
  - 过期时间倒计时
  - 设备绑定情况

- [x] 实现卡密绑定功能
  - 绑定弹窗
  - 表单验证
  - 调用绑定 API

- [x] 实现设备解绑功能
  - 二次确认弹窗
  - 调用解绑 API

---

## 阶段四：管理员核心业务（Admin Only）

### ✅ 任务 10：卡密管理模块 **已完成**

**目标**：全能的卡密生命周期管理

**⭐ 使用 ui-ux-pro-max skill 美化卡密管理页**

**接口信息**：

1. **查询卡密列表**：
   ```http
   GET /api/v1/admin/cards?page={page}&size={size}&app_id={app_id}&status={status}&keyword={keyword}
   Authorization: Bearer {admin_token}

   返回值:
   {
     "total": number,
     "page": number,
     "size": number,
     "cards": [
       {
         "id": number,
         "app_id": number,
         "app_name": string,
         "card_key": string,
         "status": "unused" | "used" | "disabled",
         "expire_time": string,
         "max_device_count": number,
         "permissions": string[] | object,
         "remark": string,
         "bind_user_count": number,
         "bind_device_count": number,
         "created_at": string
       }
     ]
   }
   ```

2. **生成卡密**：
   ```http
   POST /api/v1/admin/card/generate
   Authorization: Bearer {admin_token}
   Content-Type: application/json

   请求参数:
   {
     "app_id": number,
     "count": number,             // 1-1000
     "expire_time": string,        // ISO 8601 格式，必须大于当前时间
     "max_device_count": number,   // 1-100，默认1
     "permissions": string[] | object,
     "remark": string              // 可选，最大255字符
   }

   返回值:
   {
     "success": boolean,
     "message": string,
     "count": number,
     "cards": string[]             // 卡密数组
   }
   ```

3. **更新卡密状态**：
   ```http
   POST /api/v1/admin/card/{card_id}/status
   Authorization: Bearer {admin_token}
   Content-Type: application/json
   
   请求参数:
   {
     "status": "unused" | "used" | "disabled"
   }
   
   返回值:
   {
     "success": boolean,
     "message": string
   }
   ```

4. **更新卡密权限**：
   ```http
   POST /api/v1/admin/card/{card_id}/permissions
   Authorization: Bearer {admin_token}
   Content-Type: application/json
   
   请求参数:
   {
     "permissions": string[]
   }
   
   返回值:
   {
     "success": boolean,
     "message": string
   }
   ```

5. **查询设备列表**（指定卡密）：
   ```http
   GET /api/v1/admin/devices?card_id={card_id}
   Authorization: Bearer {admin_token}

   返回值:
   {
     "total": number,
     "devices": [
       {
         "id": number,
         "card_id": number,
         "card_key": string,
         "device_id": string,
         "device_name": string,
         "bind_time": string,
         "last_active_at": string,
         "status": "active" | "disabled"
       }
     ]
   }
   ```

**操作清单**：

- [x] 创建卡密管理页面 (`src/views/card/index.vue`)

- [x] 开发卡密列表表格
  - 高级筛选（应用、状态、关键词）
  - 分页组件
  - 状态标签颜色区分
  - 操作列（禁用/启用、修改权限、查看设备）

- [x] 开发卡密生成弹窗 (`src/views/card/components/GenerateDialog.vue`)
  - 表单字段：
    - 应用选择（下拉框）
    - 生成数量（数字输入）
    - 过期时间（日期时间选择器）
    - 最大设备数（数字输入）
    - 权限选择（多选框）
    - 备注（文本输入）
  - 表单验证
  - 调用生成 API
  - 生成结果展示（支持复制、导出）

- [x] 开发权限修改弹窗 (`src/views/card/components/PermissionDialog.vue`)
  - 权限多选框
  - 调用更新权限 API

- [x] 开发设备查看弹窗 (`src/views/card/components/DeviceDialog.vue`)
  - 设备列表表格
  - 设备状态标签
  - 最后活跃时间
  - 操作：禁用/启用设备

- [x] 实现状态管理功能
  - 二次确认弹窗
  - 调用更新状态 API

---

### ✅ 任务 11：用户管理模块 **已完成**

**目标**：管控平台用户

**⭐ 使用 ui-ux-pro-max skill 美化用户管理页**

**接口信息**：

1. **查询用户列表**：
   ```http
   GET /api/v1/admin/users?page={page}&size={size}&status={status}&keyword={keyword}
   Authorization: Bearer {admin_token}

   返回值:
   {
     "total": number,
     "page": number,
     "size": number,
     "users": [
       {
         "id": number,
         "username": string,
         "status": "normal" | "banned",
         "role": "admin" | "user",
         "card_count": number,          // 有效卡密数量
         "created_at": string,
         "last_login_at": string
       }
     ]
   }
   ```

2. **更新用户状态**：
   ```http
   POST /api/v1/admin/user/{user_id}/status
   Authorization: Bearer {admin_token}
   Content-Type: application/json

   请求参数:
   {
     "status": "normal" | "banned"    // Query参数，不是body参数
   }

   返回值:
   {
     "success": boolean,
     "message": string
   }
   ```

**操作清单**：

- [x] 创建用户管理页面 (`src/views/user/index.vue`)

- [x] 开发用户列表表格
  - 筛选（状态、关键词搜索）
  - 分页
  - 列：ID、用户名、角色、状态、有效卡密数、创建时间、最后登录时间、操作

- [x] 实现用户封禁/解封功能
  - 状态列高亮显示（正常-绿色、封禁-红色）
  - 二次确认弹窗
  - 调用更新状态 API

---

### ✅ 任务 12：设备管理模块 **已完成**

**目标**：监控与管理在线设备

**⭐ 使用 ui-ux-pro-max skill 美化设备管理页**

**接口信息**：

1. **查询设备列表**：
   ```http
   GET /api/v1/admin/devices?page={page}&size={size}&card_id={card_id}&user_id={user_id}&status={status}
   Authorization: Bearer {admin_token}

   返回值:
   {
     "total": number,
     "page": number,
     "size": number,
     "devices": [
       {
         "id": number,
         "card_id": number,
         "card_key": string,
         "device_id": string,
         "device_name": string,
         "bind_time": string,
         "last_active_at": string,
         "status": "active" | "disabled"
       }
     ]
   }
   ```

2. **更新设备状态**：
   ```http
   POST /api/v1/admin/device/{device_id}/status
   Authorization: Bearer {admin_token}
   Content-Type: application/json
   
   请求参数:
   {
     "status": "active" | "disabled"
   }
   
   返回值:
   {
     "success": boolean,
     "message": string
   }
   ```

**操作清单**：

- [x] 创建设备管理页面 (`src/views/device/index.vue`)

- [x] 开发设备列表表格
  - 筛选（卡密、用户、状态）
  - 分页
  - 列：设备ID、设备名称、关联卡密、关联用户、绑定时间、最后活跃时间、状态、操作

- [x] 实现设备禁用/启用功能
  - 二次确认弹窗
  - 调用更新状态 API

- [x] 开发活跃状态指示器
  - 显示最后活跃时间
  - 活跃状态颜色标识（绿色-活跃、灰色-离线）

---

### ✅ 任务 13：应用管理模块 **已完成**

**目标**：多应用接入支持

**⭐ 使用 ui-ux-pro-max skill 美化应用管理页**

**接口信息**：

1. **查询应用列表**：
   ```http
   GET /api/v1/app/list
   Authorization: Bearer {admin_token}
   
   返回值:
   {
     "total": number,
     "apps": [
       {
         "id": number,
         "app_key": string,
         "app_name": string,
         "status": "normal" | "disabled",
         "created_at": string
       }
     ]
   }
   ```

2. **创建应用**：
   ```http
   POST /api/v1/app/create
   Authorization: Bearer {admin_token}
   Content-Type: application/json
   
   请求参数:
   {
     "app_name": string,
     "app_key": string        // 可选，不填自动生成
   }
   
   返回值:
   {
     "success": boolean,
     "message": string,
     "app": {
       "id": number,
       "app_key": string,
       "app_name": string,
       "status": "normal",
       "created_at": string
     }
   }
   ```

3. **更新应用状态**：
   ```http
   POST /api/v1/app/{app_id}/status
   Authorization: Bearer {admin_token}
   Content-Type: application/json
   
   请求参数:
   {
     "status": "normal" | "disabled"
   }
   
   返回值:
   {
     "success": boolean,
     "message": string,
     "app": { ... }
   }
   ```

**操作清单**：

- [x] 创建应用管理页面 (`src/views/app/index.vue`)

- [x] 开发应用列表表格
  - 列：ID、应用名称、AppKey、状态、创建时间、操作

- [x] 开发创建应用弹窗
  - 表单字段：应用名称、AppKey（可选）
  - 自动生成 AppKey 功能
  - 调用创建 API

- [x] 实现应用状态切换
  - 二次确认弹窗
  - 调用更新状态 API

---

## 阶段五：错误页与扩展功能

### ✅ 任务 14：错误页面开发

**目标**：完善的异常流程体验

**⭐ 使用 ui-ux-pro-max skill 美化错误页**

**操作清单**：

- [ ] 开发 403 Forbidden 页面 (`src/views/error/403.vue`)
  - 403 图标/插画
  - 提示文字："抱歉，您无权访问该页面"
  - 返回首页按钮
  - 退出登录按钮

- [ ] 开发 404 Not Found 页面 (`src/views/error/404.vue`)
  - 404 图标/插画
  - 提示文字："页面不存在"
  - 返回首页按钮

- [ ] 配置全局错误处理
  - 网络错误提示（Toast）
  - 接口错误统一拦截

---

### ✅ 任务 15：统计数据页面（普通用户可见）

**目标**：为普通用户提供统计数据视图

**⭐ 使用 ui-ux-pro-max skill 美化统计页**

**接口信息**：
```http
GET /api/v1/admin/statistics
Authorization: Bearer {token}

返回值: 同任务8
```

**操作清单**：

- [ ] 创建统计数据页面 (`src/views/stats/index.vue`)
  - 与仪表盘类似的展示
  - 普通用户只显示与自己相关的数据
  - 图表可视化（可选）

---

## 阶段六：UI/UX 极致优化

### ✅ 任务 16：视觉与动画增强

**目标**：提升"高级感"

**⭐ 全面使用 ui-ux-pro-max skill 进行优化**

**操作清单**：

- [ ] 添加页面切换过渡动画
  ```vue
  <router-view v-slot="{ Component }">
    <transition name="fade-slide" mode="out-in">
      <component :is="Component" />
    </transition>
  </router-view>
  ```
  
  ```css
  .fade-slide-enter-active,
  .fade-slide-leave-active {
    transition: all 0.3s ease;
  }
  
  .fade-slide-enter-from {
    opacity: 0;
    transform: translateX(30px);
  }
  
  .fade-slide-leave-to {
    opacity: 0;
    transform: translateX(-30px);
  }
  ```

- [ ] 优化列表加载状态
  - 实现 Skeleton 骨架屏
  - Loading 动画

- [ ] 优化按钮点击反馈
  - Ripple 波纹效果
  - 点击缩放动画

- [ ] 优化弹窗出现/消失动画
  - 缩放 + 淡入淡出
  - 背景模糊效果

- [ ] 统一全站视觉规范
  - 圆角：`rounded-lg` (8px)
  - 阴影：`shadow-sm` / `shadow-md`
  - 间距：4px 的倍数（8px、16px、24px、32px）

---

### ✅ 任务 17：响应式适配

**目标**：移动端友好

**操作清单**：

- [ ] 适配移动端侧边栏
  - 小屏幕自动折叠
  - 抽屉式菜单（Drawer）

- [ ] 适配表格在小屏下的显示
  - 横向滚动
  - 或卡片式布局

- [ ] 调整移动端交互
  - 字体大小适配
  - 点击区域增大（至少 44x44px）

---

### ✅ 任务 18：构建与部署

**目标**：生产环境准备

**操作清单**：

- [ ] 生产环境构建配置
  ```bash
  npm run build
  ```
  
  优化 `vite.config.ts`:
  ```ts
  import { defineConfig } from 'vite'
  import vue from '@vitejs/plugin-vue'
  import AutoImport from 'unplugin-auto-import/vite'
  import Components from 'unplugin-vue-components/vite'
  import { ElementPlusResolver } from 'unplugin-vue-components/resolvers'
  import { resolve } from 'path'
  
  export default defineConfig({
    plugins: [
      vue(),
      AutoImport({
        resolvers: [ElementPlusResolver()]
      }),
      Components({
        resolvers: [ElementPlusResolver()]
      })
    ],
    resolve: {
      alias: {
        '@': resolve(__dirname, 'src')
      }
    },
    build: {
      rollupOptions: {
        output: {
          manualChunks: {
            'element-plus': ['element-plus'],
            'vue': ['vue', 'vue-router', 'pinia']
          }
        }
      },
      chunkSizeWarningLimit: 1000
    }
  })
  ```

- [ ] 编写部署文档 (`DEPLOY.md`)
  - 环境要求
  - 构建步骤
  - Nginx 配置示例
  - 域名配置

- [ ] 编写开发文档 (`README.md`)
  - 项目介绍
  - 技术栈
  - 目录结构
  - 开发指南
  - API 文档链接

---

## 📝 开发提示

### UI/UX 设计原则

1. **使用 ui-ux-pro-max skill 进行页面美化**
   - 每个页面开发前，先规划布局和交互
   - 参考现代设计系统（如 Ant Design、Material Design）
   - 保持视觉一致性

2. **颜色使用**
   - 主色调：蓝色系（#3B82F6）
   - 成功：绿色（#10B981）
   - 警告：黄色（#F59E0B）
   - 危险：红色（#EF4444）
   - 中性：灰色系

3. **交互反馈**
   - 所有操作都要有 loading 状态
   - 成功/失败都要有提示
   - 危险操作必须二次确认

4. **性能优化**
   - 使用虚拟滚动（长列表）
   - 图片懒加载
   - 路由懒加载
   - 组件按需加载

### 开发流程建议

1. 每完成一个任务，勾选对应复选框
2. 遇到后端接口变更时，优先更新 `src/api` 和 `src/types`
3. 始终保持"高颜值"标准，拒绝原生丑陋的 UI
4. 定期 code review，保持代码质量
5. 及时编写注释和文档

### 测试建议

- [ ] 功能测试：每个功能开发完成后立即测试
- [ ] 兼容性测试：Chrome、Firefox、Safari
- [ ] 响应式测试：桌面端、平板、手机
- [ ] 性能测试：Lighthouse 评分 > 90

---

> **最后更新**: 2026-01-29
>
> **文档版本**: v2.0
>
> **参考文档**:
> - `管理后台整体布局与交互设计说明.md`
> - `管理后台前端设计说明（Vue3+TypeScript）.md`
> - `通用卡密与授权系统任务.md`
> - `API接口速查表.md`
