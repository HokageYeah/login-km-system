...
- **阶段四**：权限校验系统（任务 11-13）✅ **已完成**
- **阶段五**：管理后台功能（任务 14-16）✅ **已完成**
- **阶段六**：增强与优化（任务 17-20）✅ **已完成**
  ...
  "cards": [
    {
      ...
      "bind_devices": 1,
      "devices": ["abc123def456"],
      "max_device_count": 2
    }
  ]
}
```
说明：`devices` 为当前卡密已绑定的活跃设备ID列表，列表元素是 `device_id` 字符串，数量与 `bind_devices` 保持一致。
...
#### 16.6 查询设备绑定列表
```http
GET /api/v1/admin/devices?card_key=AP4H-JJFQ-8UCB-BRD4&username=testuser
Authorization: Bearer {admin_token}
```
...
1. **邮箱验证码**：注册时验证邮箱
2. **付费/续费系统**：集成支付接口
3. **多卡密切换**：一个用户可绑定多张卡密，权限校验优先按当前卡密隔离；未传当前卡密时保留按用户和设备查询的兼容兜底
4. **订阅制授权**：按月/年订阅模式
