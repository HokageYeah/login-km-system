  ...
  "cards": [
    {
      ...
      "app_key": "default_app",
      "app_status": "normal",
      "app_created_at": "2026-01-01T00:00:00"
    }
  ]
}
```
...
说明：
- `is_expired` 由服务端按 `expire_time` 动态计算，仅表示当前是否过期；原有 `status` 字段语义不变。
- `devices` 为当前卡密已绑定的活跃设备ID列表，只返回 `device_id` 字符串，数量与 `bind_devices` 保持一致。
...
说明：`card_id` 是当前使用的卡密ID。传入后，服务端只按“当前用户 + 当前设备 + 当前卡密”裁决权限；不传时，才按当前用户和设备查询可用卡密作为兼容兜底。
...
应用管理页现在不仅负责创建和启停应用，还承担应用维度的导航入口。
...
- 点击“卡密数”会跳转到卡密管理页，并自动带上当前 `app_id`
- 点击“权限数”会跳转到功能权限管理页，并自动带上当前 `app_id`

这样做的目的，是把“应用”作为后台的一级管理维度，避免管理员需要先记住应用名，再到其他页面重复手工筛选。

### 功能权限管理页怎么用

功能权限管理页现在采用“权限归属应用”的设计：

1. 创建权限时必须先选择所属应用
2. 如果系统里还没有任何应用，页面会提示先去应用管理页创建应用
3. “所属应用”列使用稳定颜色标签，不同应用颜色不同，但同一应用在不同页面颜色保持一致
4. 支持勾选多条权限后执行批量删除
...
- 删除的是权限中心中的元数据记录
- 不会自动去改历史卡密中的权限 JSON
- 这样做是为了避免在清理后台展示数据时误伤已经发放出去的授权记录

### 卡密管理页中的应用筛选

卡密管理页已经支持通过路由参数自动接收 `app_id` 筛选。

也就是说：

- 从应用管理页点击某个应用的“卡密数”跳转过去时
- 页面会自动只展示当前应用下的卡密
- 管理员不需要再次手工选择应用

同理，功能权限管理页也支持相同的路由筛选逻辑。
...
说明：管理后台用户页的“有效卡密”列会调用该接口，并支持从弹窗点击单张卡密跳转到卡密管理页。
...
```bash
curl -X POST "http://localhost:9999/api/v1/admin/card/42/expire-time" \
  -H "Authorization: Bearer ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "expire_time": "2027-12-31T23:59:59"
...
说明：该接口只更新 `cards.expire_time`，不会新增或修改卡密状态枚举；是否已过期仍由服务端按当前时间动态判断。选择过去时间可以让卡密立即进入已过期状态。

### 修改卡密最大设备数

```bash
curl -X POST "http://localhost:9999/api/v1/admin/card/42/max-device-count" \
  -H "Authorization: Bearer ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "max_device_count": 3
...
- 该接口直接更新 `cards.max_device_count`，修改后会立即影响后续设备绑定校验；
- 新的设备上限不能小于当前活跃设备绑定数，否则接口会拒绝，避免出现“已绑定设备数大于配置上限”的不一致状态；
- 管理后台卡密列表中的“查看设备”弹窗已经提供该操作入口，可在查看设备时直接调整设备限制。
...
```bash
# 按卡密字符串和用户名筛选设备
curl -X GET "http://localhost:9999/api/v1/admin/devices?card_key=AP4H-JJFQ-8UCB-BRD4&username=testuser" \
  -H "Authorization: Bearer ADMIN_TOKEN"
```

说明：`card_key`、`username` 为当前推荐筛选参数；`card_id`、`user_id` 继续兼容旧调用，可按需传递。
...
2. 检查是否绑定了卡密
   ...
   - 如果当前账号角色为 `admin`，则不受卡密绑定限制，可跳过此项
...
**导出功能权限快照**:
```bash
curl -X POST "http://localhost:9999/api/v1/admin/feature-permissions/export" \
  -H "Authorization: Bearer ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "permission_keys": ["wechat", "ximalaya"]
  }' \
  -o feature_permissions.json
```

说明：

1. 导出只会包含后台列表中勾选的权限，导出范围更明确
2. 导出文件是标准 JSON，可直接用于其他服务器导入
3. 文件内容不包含数据库 ID，跨环境导入时按 `permission_key` 做幂等同步

**导入功能权限快照**:
```bash
curl -X POST "http://localhost:9999/api/v1/admin/feature-permissions/import" \
  -H "Authorization: Bearer ADMIN_TOKEN" \
  -F "file=@feature_permissions.json"
```

说明：

1. 导入时系统会自动解析导出文件并校验格式
2. 如果目标服务器已存在相同 `permission_key`，则更新名称、描述、价格、分类、图标、排序和状态
3. 如果目标服务器不存在该 `permission_key`，则自动创建
4. 导入成功后，管理后台权限列表页面会刷新显示最新数据
