...
- 批量生成卡密请求包含 `price` 字段，表示生成后写入 `cards.price` 的最终售卖价格；
- 管理后台页面会按“权限月价按天折算 + 超出 3 台后的设备固定加价”计算建议价，但管理员可在生成前手动调整最终价格；
- 当管理员修改卡密的权限、过期时间、最大设备数时，服务端会按同一规则重新计算并回写 `cards.price`。
...
- 设备列表中的每条记录会返回所属卡密的 `price`，便于后台直接按设备视角查看当前绑定卡密售价。

### 统计看板
| 方法 | 路径 | 说明 | 权限 |
|------|------|------|------|
| GET | `/api/v1/admin/statistics` | 获取综合统计、收入统计、销售趋势、权限收入归因和资源趋势，支持 `start_date`、`end_date`、`trend_start_date`、`trend_end_date` | 🔑 管理员 |

补充说明：

- `revenue` 汇总使用中与未使用卡密价格，禁用卡密不计入收入；
- `all_time_revenue` 汇总全时间使用中与未使用卡密价格，用于总收入总盘展示；
- `trend_range` 返回综合看板资源趋势区的实际统计范围；
- `sales_trend` 按卡密生成日期返回每日销售额、订单数和客单价；
- `permission_revenue` 将卡密最终价格按卡密包含的权限均分归因，便于观察叠加权限的收入贡献。
- `start_date`、`end_date` 格式为 `YYYY-MM-DD`；不传时默认统计当天收入数据。
- `trend_start_date`、`trend_end_date` 格式为 `YYYY-MM-DD`；不传时默认统计当天资源趋势数据，前端“指定月份”模式会自动换算整月起止日期后再传入。
...
- 功能权限元数据包含 `price` 售卖价格，创建、编辑、列表和导入导出统一使用该字段；
